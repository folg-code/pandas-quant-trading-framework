...
def candlestick_confirmation(df):
    ...
    prev3_close = close.shift(3)
    prev3_open = open_.shift(3)
...
def rma(dataframe, source, period):
    df = dataframe.copy()
    sma3 = sma(source, int(period * 3))
    sma2 = sma(source, int(period * 2))
    sma1 = sma(source, int(period * 1))
